<template>
  <table id="customers">
    ...
    <tr
      ...
      :key="index"
      @click="goToCentro(centro)"
    >
      <td>{{ centro.id_center }}</td>
      <td>{{ centro.unap }}</td>
      <td>{{ centro.region }}</td>
  ...
  <!-- <h6 class="noRegist" v-if="!this.totales.centros">
...
// import { updateOne } from "@/services/centro/Centro";
...
export default {
  ...
  methods: {
    goToCentro(centro: any) {
      this.$store.state.user.currentCentro = centro;
      this.$router.push(`/centros/details`);
    },
    ...
    // async marcarListo(centro: Centro) {
    //   alert("Centro Lista.");
    //   try {
    //     const res = await updateOne(centro);
    //   } catch (error) {
    //     console.error(error);
    //   }
    // },
