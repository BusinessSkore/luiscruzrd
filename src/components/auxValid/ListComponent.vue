<template>
  ...
  <table id="customers">
    ...
    <tr v-for="(evaluacion, index) in evaluacions" :key="index">
      ...
      <td :class="hasDxColor(evaluacion.evaluacion)">
      ...
      <td>
        <i
          @click="printEvaluacionScan(evaluacion)"
...
export default {
  ...
  methods: {
    printEvaluacionScan(evaluacion: any) {
      let listToPrint = [];
      listToPrint.push(evaluacion);
      this.$store.state.user.listToPrint = listToPrint;
      this.$router.push("/printEvaluacionScan");
    },
    goToEval(evaluacion: any) {
      this.$store.state.user.currentEvaluacion = evaluacion;
      this.$router.push(`/verifications_details`);
    },
